...
//synrgbwir::synrgbwir(std::string image_dir, std::string imu_dir, std::string gt_dir)
synrgbwir::synrgbwir(std::string path )
{
    cout << "class synrgbwir created a object: " << endl;
    color_sub_ = new message_filters::Subscriber<sensor_msgs::Image>(nh, "/kinect2/qhd/image_color_rect", 10);
    depth_sub_ = new message_filters::Subscriber<sensor_msgs::Image>(nh, "/kinect2/qhd/image_depth_rect", 10);
    ir_sub_ = new message_filters::Subscriber<sensor_msgs::Image>(nh, "/optris/thermal_image", 10);
    gt_sub_ = new message_filters::Subscriber<geometry_msgs::PoseStamped>(nh, "/vrpn_client_node/RigidBody/pose", 10);
    
    sync_ = new message_filters::Synchronizer<MySyncPolicy>(MySyncPolicy(10), *color_sub_, *depth_sub_, *ir_sub_, *gt_sub_);
    ...
    //创建文件
    ImgStamp.open( (gt_dir_ + "ImgStamp.csv" ).c_str(), ios::app );
    ...
    sync_->registerCallback(boost::bind(&synrgbwir::CallBack, this, _1, _2, _3, _4));
}
...
void synrgbwir::CallBack(const sensor_msgs::ImageConstPtr &colorImage, const sensor_msgs::ImageConstPtr &depthImage, const sensor_msgs::ImageConstPtr &IR, const geometry_msgs::PoseStampedConstPtr &gt)
{
    cout << "callback:" << endl;
    ...
    ImgStamp << colorImage->header.stamp << "," << depthImage->header.stamp << "," << IR->header.stamp << "," << gt->header.stamp <<"\n";
